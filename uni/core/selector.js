...
// 	Add functionality to uni implementing selector engine.  
//	Note that all methods except #match will set the Subject.
//
...
//	@param		{Object}		[p]		A parent element within which to run #find.
//	@type		{DOMElement}
//
uni.hoist('findFirst', function(s, p) {
	return uni.ns._.Dom.select(s, p)[0];
});

//	#select
//  
//	Returns a (js) array of matched elements, empty if none were found.
//
//	@param		{String}		s		A valid css selector.
//	@param		{Object}		[p]		A parent element within which to run #search.
//	@type		{Array}
//
uni.hoist('select', uni.ns._.Dom.select);
...
// 	Retrieve element by id attr.
//
uni.hoist('byId', uni.ns._.Dom.byId);

// Retrieve elements by tag name.
//
uni.hoist('byTag', uni.ns._.Dom.byTag);

// Retrieve elements by name attr.
//
uni.hoist('byName', uni.ns._.Dom.byName);

// Retrieve elements by class name.
//
uni.hoist('byClass', uni.ns._.Dom.byClass);
...
//
//	@param		{String}		s		A valid css selector.
//	@param		{DOMElement}	e		A Dom element.  Default is Subject.
//	@type		{Boolean}
//
//	Because #match can be used very often (such as with %event kit), and given that it returns
//	a non-chainable value, we add it to the prototype, to avoid a flood of reporting.
//
uni.extendProto('match', function(s, e) {
	return uni.ns._.Dom.match(e || this.$, s);
});